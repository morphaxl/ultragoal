...
**Size the fan-out by what parallelism actually buys.** Parallel subagents pay on decomposable, *read-heavy* work — research, search, audits — where independent threads don't share decisions; they pay nothing on sequential build work, where actions carry implicit decisions and parallel workers produce conflicting ones. Sizing guide: a simple lookup gets no subagent, a comparison or multi-file scan gets 2–4 scouts, only genuinely open-ended research earns more — three focused subagents routinely outperform five scattered ones, and each agent costs roughly linear tokens. For verification the scaling axis is *aspects, not repeats*: distinct lenses catch what identical re-runs cannot — which is what `verify: panel` packages. Recommend panel whenever the result genuinely matters — high-stakes, destructive, or long autonomous goals — and say what it adds in the recap's "what it will take" line (about two extra verifier passes at the final sign-off). Choose machinery for the result, not the token bill; the user's lever is informed consent at arming time, not a quietly smaller plan.
...
- Never check a rubric box on your own say-so, and never write a `ULTRAGOAL-VERIFIED` line yourself — that verdict is the verifier's alone. Dispatch the `ultragoal:verifier` subagent at the cadence set in `.ultragoal/config.md` (default: before claiming any rubric item), **passing it the exact path to this goal's `goal.md`** so it hashes and signs the right file. It re-runs the checks and appends the verdict. Check boxes only for items the verifier passed, and when you check a box, record an indented evidence line directly under it — `- evidence: \`command\` -> key output line` — the verifier audits this ledger first and treats a checked box without evidence as an automatic FAIL. Pipeline it: dispatch the verifier on finished items in the background and keep building the next one while it checks — act on the verdict when it lands instead of idling for it. On `verify: panel` goals, the FINAL sign-off is a panel: dispatch three verifier subagents in ONE message (parallel dispatch keeps them mutually blind), assigning one lens each — `checks`, `refute`, `constraints`; each appends its own lens-tagged verdict, and the gate releases only when the most recent verdict for all three lenses is a PASS on the current rubric. Panel cost is bounded by design — it fires once per goal, roughly two extra verifier dispatches; keep it that way (interim claims stay on the single verifier), and run the mechanical `checks` lens on a cheaper model (small judges match large ones on command re-runs) while `refute` gets the strongest available.
