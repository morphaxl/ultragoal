...
- Glance at the tail of `.ultragoal/stats.tsv` (last ~10 rows): a domain with a clean history earns a confident vanilla/quick recommendation at the dials; repeated verifier fails or blown budgets in similar goals earn a standard-rigor or deeper-budget recommendation — and say which history drove the call when you offer the dials. Autonomy is earned from the record, not assumed.

**Off-ramp — not everything needs the loop.** If the consult shows a contained, reversible change you could finish and verify with a couple of commands — no real forks, no long unattended run — say so in one line and do the work directly in this conversation instead of arming a goal: same evidence discipline, no spec, no interview, no ceremony. The machinery exists for work a plain session can't hold; spending more tokens on ceremony than on the work is a failure mode. Arm anyway only if the user says they want the loop (the audit trail, or to walk away mid-run). This applies doubly after a goal has finished in this session: a completed goal does not put the session in "goal mode" — later small asks get ordinary treatment (no verifier dispatch, no evidence ledger, no rubric), and only an explicit next-round brief re-enters the loop.

**Program on-ramp — the off-ramp's mirror.** When the brief is bigger than one goal should hold — it would blow past the deep tier, spans different verification regimes (backend contracts vs UI vs docs), or contains a risky unknown whose findings should reshape the later work — don't draft one bloated spec. Propose a **program**: an ordered chain of goals with the cut points named and justified (each cut buys a verified checkpoint plus a fresh context that rebuilds from the brief, the distilled memory, and the real repo state), run one at a time, with only provably-disjoint spans overlapping in worktrees. Interview once at the program level, present the program map, and ask a standalone "Arm program" question. The full protocol — division heuristics, blast-radius rules, checkpoint replanning, the manager's own goal — is in [docs/programs.md](../../docs/programs.md); read it before proposing one. Most briefs are goal-sized; a program is as rare as the off-ramp is common.
...
Copy the structure from [goal-template.md](goal-template.md) and write the rubric following [rubric-guide.md](rubric-guide.md) — read it; rubric quality decides whether this loop converges. Also read [qa-capability-map.md](qa-capability-map.md) before finalizing the rubric. For every claim, choose the proof rung that matches the promise: static checks for wiring, tests for pure logic, browser/simulator screenshots or rendered-size assertions for UI pixels, click/deep-link checks for reachability, live/staging smokes for external seams, and paired failure-mode checks for new boundaries. A well-designed rubric is doing more work than the model.

Before showing the user, adversarially review your own rubric against the guide's full anti-pattern list. The mechanical audit (Phase 4) catches the structural defects; this read catches the judgment ones the linter can't — above all, claims proved one rung below where they live (wired ≠ renders ≠ reachable ≠ live: the guide's evidence ladder). Sequence at least one live, end-to-end slice *early* in the rubric rather than deferring all real exercise to a final manual round. Fix what you find.

**Set the rigor — scale the loop to the model, and let the user choose per run.** `rigor` has a default in `.ultragoal/config.md` (**vanilla** if unset), but it's a per-goal choice — offer it as a dial at arm time, in the **same `AskUserQuestion` batch as the depth dial below** (they're the two effort dials, and a user steering one usually wants to steer the other): the config value is the recommended default (one tap to accept), and the user can dial up or down for *this* run without editing config. Skip the question only when the brief already settles it ("max mode" / "vanilla" / "standard") or the goal took the off-ramp. Rigor sets how much scaffolding the harness adds to compensate for model strength — it picks the loop's *baseline*, which the stakes × ambiguity × length sizing below then modulates. When in doubt between modes, recommend the higher one and say what it buys:
...
- **standard**: keep the single grader, add safety nets. Dispatch an interim verifier on any item that already failed or whose check looks shaky; pessimistic double-runs on near-threshold checks; 2–4 parallel scouts for decomposable read-heavy work (research, audits) — and give each scout an explicit cheap model (e.g. `model: sonnet` or `haiku`): the Explore agent inherits the session's model now, so an unhinted scout costs frontier rates for lookup work. The background log monitor is active. `verify: on`.
...
**The rigor dial** is the "Set the rigor" question above, asked in this same batch — phrase the three modes in human terms (lean and fast / safety nets / release-grade) with the config default first; the pick sets the spec's `verify:` (vanilla / standard → `on`, max → `panel`) and selects the loop kit already described.

**The execution dial — plan big, execute small.** For an execution-heavy goal (many files to change, a large build, long mechanical stretches) on a frontier session model, offer a third dial in the same batch — config default first (`execution` in `.ultragoal/config.md`, **frontier** if unset):

- **Frontier** — the session model implements everything itself. Right for small or subtle goals, where delegation overhead outweighs any savings.
- **Economy** — the session model stays orchestrator and advisor: it interviews, specs, decides, reviews, and dispatches verification — and delegates the implementation chunks to cheaper executor subagents (`model: sonnet`), each against its own mini-rubric, reviewing every returned diff before its box is checked. An executor that gets stuck escalates back to the parent — that escalation *is* the advisor call. This is Anthropic's published cost pattern: a Fable 5 orchestrator with Sonnet 5 workers kept 96% of Fable's BrowseComp score at 46% of the price; the inverse advisor shape kept ~92% of SWE-bench Pro at ~63%.

Skip the dial entirely when execution won't dominate the goal's cost — a dial that can't change the bill is ceremony. Economy never touches the loop's judgment surfaces: the interview, the spec, rubric design, diff review, and the verifier all stay on the strong model — cheap executors are safe *because* the checking around them isn't cheap. Record the pick in the spec's Context.
...
Resolve `<this-skill-dir>` to the directory containing this `SKILL.md`; in the ultragoal source repo the equivalent shorthand is `node scripts/rubric-audit.mjs <goal>`. If the script is unavailable, run the guide's anti-pattern list against the draft by hand. A `BLOCKER` is a draft defect: revise the draft and run the audit again before recapping — the arm-guard hook runs this same audit and blocks the arm question while BLOCKERs remain. A `WARN` must either be fixed or recorded in the draft's Context as an intentional tradeoff; do not hide warnings from yourself and then ask the user to arm a weak contract.
...
**For any goal that adds or changes a screen, destination, or navigation, lead the recap with a "What you'll see" block** — read [ui-recap.md](ui-recap.md) and build it as it describes: nav map, screen inventory, ASCII wireframe, flow, and stated assumptions, all from the user's chair, never what the code does. It replaces the prose of part 3 for UI work, travels into the finish summary and any handoff prompt, and is skipped entirely for backend/refactor goals, which instead confirm their own end-state artifact (an endpoint signature, the resulting file tree, a sample input→output).
...
- Delegate independent subtasks to subagents and keep working while they run — they run in the background by default now, so dispatch and continue rather than idling; a subagent cut off by rate limits returns failure or partial work, never a false success, so treat a subagent error as "re-dispatch or do it yourself", not as done. Fan out only on genuinely independent seams (separate files, modules, subtopics), each subagent against its own mini-rubric with the parent owning verification — subagents don't get the gate. In economy mode, implementation chunks route to the cheap executors by default (the dial's whole point): give each a tight brief + mini-rubric, review every returned diff yourself before checking any rubric box, and treat an executor's escalation or failure as your work item, not a reason to lower the bar. Skip fan-out for a tightly-coupled deliverable, where parallel agents collide on one artifact; 2–4 focused beat five. For many similar pieces, **dynamic workflows** (scripted, deterministic fan-out) are worth reaching for, with the user's explicit opt-in.
