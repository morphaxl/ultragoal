...
1. **Every item checkable by a command, never vibes.** The check must be runnable in this repo, and its output must decide the item unambiguously.
   ...
   - **Rubric from exemplar.** When the repo (or the user) has a known-good artifact of the same kind — a screen that already feels right, an endpoint with the error handling you want everywhere, a doc page of the target quality — derive items from it: analyze what makes the exemplar good, turn each property into a checkable line, and cite the exemplar in the item ("error envelope matches `api/users`' shape — check: `diff <(...) <(...)`"). An exemplar grounds thresholds in something real instead of invented numbers, and Anthropic's own Outcomes guidance recommends exactly this move.
...
5. **Constraints are part of the rubric.** Anything that must NOT change goes in Constraints, and the verifier checks them: "no other test file modified — check: `git diff --stat main -- 'test/**'`". Every constraint needs a number, a never, or a command that checks it — anything softer gets optimized away.
