...
Three more settings with sensible defaults — mention they exist in `config.md` rather than asking: default turn budget (25), verification cadence (final — one verifier pass at sign-off; "every-claim" verifies before each box for stricter, costlier loops), and interview depth (adaptive — the goal skill sizes the interview to the goal's stakes and length; "quick" forces one batch, "deep" forces multi-round).
