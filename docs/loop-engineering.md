...
- **Verification is hash-bound — to the checks, not to progress.** The verifier recomputes the rubric's checksum itself — it never accepts one quoted to it — re-runs the checks, and appends `ULTRAGOAL-VERIFIED: PASS rubric=<hash>` to the goal file. The hash normalizes out checkbox state and evidence lines, so marking items done never voids a verdict, while editing what any check says or measures always does. The gate only accepts a verdict whose hash matches the current rubric.
